...
Check out this extended tutorial about RobotCode:
...
Popular plugins for Robot Framework:
- [RobotCode](https://plugins.jetbrains.com/plugin/26216-robotcode--robot-framework-support)
...
- The one recommended by JetBrains, using the [Toolbox App](https://www.jetbrains.com/toolbox-app/).
...
:::tip  Only install one Robot Framework Extension
Make sure that only a **single** Robot Framework plugin is installed.
We strongly recommend to only install [RobotCode](https://robotcode.io).
:::
...
4. Search for `RobotCode` and click on `Install`
5. Add a Debug Configuration for Robot Framework to run the current test suite
![Debug Config Current Test Suite](/img/getting_started/pycharm_conf_current_suite.png)
6. Add a Debug Configuration for Robot Framework to run the current test case (via selected text)
